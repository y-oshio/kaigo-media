...
> **⚠️ V3 による上書きあり(2026-07-05)**: 本章で「変更不要」と判定した writer / fact-checker / internal-linker / schema-generator は、Knowledge Base 導入(V3 33章 §3)により KB対応フェーズで「要修正」となる。本章だけを読んで実装しないこと(V3 > V2)。

> 全43ファイル(`find` 実測。初稿の「58」は誤記)を対象に、機械検索(`過去問|kakomon|drill`)+構成レビューで判定した。
> **結論: 記事パイプラインは設計時点で C1(過去問)を対象外としていたため、ほぼ無傷。** 修正は「文言の読み替え」数ファイル+「凍結」1ファイル、追加は「ドリル生成パイプライン」一式+「統計ページ解説パイプライン」一式(新規)。
...
| **drill-batch.md / drill-batch.workflow.js(新規)** | **追加** | ドリル生成パイプライン: 論点選定 → クリーンルーム生成(question-writer)→ 制度ファクトチェック → **機械的類似度チェック(26章 §5 工程4: n-gram/埋め込みスクリーニング→高類似のみ目視)** → 監修依頼キュー → status 管理。着手は P8(23章)のタイミングでよい |
| **stat-page-batch.md(新規)** | **追加** | 統計 pSEO ページの独自ブロック(解説400字以上・全国比・前年差)の生成 → 数値整合チェック(表示統計値と本文記述の突合)→ 検品キュー。**P6 着手時に作成**(パネル指摘 M-04: 600ページ×10〜15分=100〜150h の制作実務を担う系統が初稿に存在しなかった)。年次統計更新時の前年差記述の一括書き換えもこの系統で行う |
...
- **軽微修正6ファイル**(README / seo-director / keyword-strategist / cta-blocks / batch-order.schema / internal-links)…記事量産を開始する前(P3 完了時)までに実施。文言のみで挙動への影響なし。
- **stat-page-batch 一式**…P6(給料統計エンジン)着手時に作成。
- **drill-batch 一式**(パイプライン+question-writer agent+テンプレ+スキーマ+チェックリスト)…P8(ドリルエンジン)着手時に作成。**それまで作らない**(1人運営の負荷管理。存在しないエンジンのパイプラインを先に作らない)。
